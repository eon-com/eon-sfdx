...
import EONLogger, { COLOR_HEADER, COLOR_INFO, COLOR_NOTIFY, COLOR_SUCCESS, COLOR_TRACE, COLOR_WARNING } from '../../eon/EONLogger';
    ...
    if (packageDependencyTree) {
      if (includedependencies && packageDependencyTree.dependency) {
        ...
        for (const dep of packageDependencyTree.dependency) {
          let isStarted = false;
          if(this.flags.start && this.flags.start !== dep.packagename && !isStarted) {
            EONLogger.log(COLOR_WARNING(`Restart the deployment at the point of package ${this.flags.start}...👆`));
            isStarted = true;
            continue;
          }

          await limiter.schedule(async () => this.deployPackageTreeNode(dep.packagename, dep.path));
