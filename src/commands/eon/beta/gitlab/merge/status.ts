...
export default class GitLabStatus extends SfdxCommand {
  ...
  private async getSingleMergeRequest(): Promise<SingleMergeRequest> {
    ...
    try {
      ...
      if (merge.status === 200 && merge.data) {
        ...
        throw new SfdxError(`Check Response Status: ${merge.status} or no data from single merge request`);
      }
    } catch (e) {
      throw new SfdxError(`No result from Gitlab for single merge request data.`);
    }
  }
  ...
  private async getSingleMergeChanges(merge: SingleMergeRequest): Promise<SingleMergeRequest> {
    ...
    try {
      ...
      throw new SfdxError(`Error. Cannot get changes from Gitlab for single merge request`);
    }
    try {
  ...
  private async getPackageInfosFromOrg(merge: SingleMergeRequest, alias: string): Promise<SingleMergeRequest> {
    ...
    try {
      ...
      if (orgUrl.includes('eon--ft')) {
        ...
        throw new SfdxError(`Error. Found no instance salesforce url for alias ${alias}`);
      }
      ...
      throw new SfdxError(`Error. Cannot get a connection to org for alias ${alias}`);
    }
    return merge;
  }
