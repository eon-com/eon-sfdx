...
export default class GitLabStatus extends SfdxCommand {
  public static description = `This command fetch the current deployment status for a merge request`

  public static examples = [`sfdx eon:gitlab:merge:status --token gitlabxxxxx --url gitlab.com --aliases dev,ft,sit --mergeid 1523`,
  ...
  public async run(): Promise<AnyJson> {
    ...
    if(mergeWithPck.packages.size > 0){
       ...
       table.push([
           ...
           { hAlign: 'center', content: mergeWithTags.packages.size === GitLabStatus.deploymentCounter.isOnFT  ? '✔️' : '❌' },
           { hAlign: 'center', content: mergeWithTags.packages.size === GitLabStatus.deploymentCounter.isOnSIT ? '✔️' : '❌' },
           { hAlign: 'center', content: mergeWithTags.packages.size === GitLabStatus.deploymentCounter.isOnPreProd? '✔️' : '❌' },
           { hAlign: 'center', content: mergeWithTags.packages.size === GitLabStatus.deploymentCounter.isOnProd ? '✔️' : '❌' },
           { hAlign: 'center', content: mergeWithTags.packages.size === GitLabStatus.deploymentCounter.isOnEWISIT ? '✔️' : '❌' }
       ])
        ...
        for (const value of jobMap.values()){
          ...
          if(value.status === 'failed'){
            value.packages = await this.getJobMetrics(value.jobId);
             ...
             if(pckHasError){
                EONLogger.log(COLOR_WARNING(`❗️This merge request contains ${value.alias} errors. Pick your Package and check the job details:`))
                EONLogger.log(COLOR_INFO(`📦 ${value.packages.join(',')}`))
                EONLogger.log(COLOR_TRACE(`Job Link: ${COLOR_INFO(value.webUrl)}`)) 
             } else {
                EONLogger.log(COLOR_INFO(`✅ This merge request contains no ${value.alias} errors.Current packages with errors:`))
                EONLogger.log(COLOR_INFO(`📦 ${value.packages.join(',')}`))
                EONLogger.log(COLOR_TRACE(`Job Link: ${COLOR_INFO(value.webUrl)}`))
             }
            }
  ...
  private async getSingleMergeRequest(): Promise<SingleMergeRequest> {
    ...
    try {
        ...
        if (merge.status === 200 && merge.data) {
            ...
            throw new SfdxError(
                `Check Response Status: ${merge.status} or no data from single merge request`
            );
        }
    } catch (e) {
        throw new SfdxError(
            `No result from Gitlab for single merge request data.`
        );
    }
  }
  ...
  private async getSingleMergeChanges(merge: SingleMergeRequest):Promise<SingleMergeRequest> {;
    ...
    try {
        ...
        throw new SfdxError(
            `Error. Cannot get changes from Gitlab for single merge request`
        );
    }
    try{
   ...
   private async getPackageInfosFromOrg(merge: SingleMergeRequest, alias: string):Promise<SingleMergeRequest> {
    ...
    try {
        ...
        if(orgUrl.includes('eon--ft')){
            ...
            throw new SfdxError(
                `Error. Found no instance salesforce url for alias ${alias}`
            );
        }
        const packageList = await connection.query<SfpowerscriptsArtifact2>(`Select Id,Name,Version__c From SfpowerscriptsArtifact2__c`);     
    for (const [key, value] of merge.packages) {
            value[orgKey] = false;
            ...
            if (
                key === 'src-core-crm' || key === 'force-app' || key === 'src-settings' || key === 'src-temp'
            ) {
                value[orgKey] = true;
                ++GitLabStatus.deploymentCounter[orgKey];
            }
        ...
        throw new SfdxError(
            `Error. Cannot get a connection to org for alias ${alias}`
        );
    }
    return merge;
   }
            ...
            if (gitLabResponse.status === 200 && gitLabResponse.data) {
                for (const job of gitLabResponse.data) {
                   if(job.name === 'Install to EDG-FT' && !jobs.get('Install to EDG-FT')){
                    jobs.set(job.name,{name: job.name,jobId: job.id,status: job.status,webUrl: job.web_url,packages: [],alias: 'FT Deployment Pipeline'})
                   } else if(job.name === 'EDG-FT-Deploy' && !jobs.get('EDG-FT-Deploy')){
                    jobs.set(job.name,{name: job.name,jobId: job.id,status: job.status,webUrl: job.web_url,packages: [],alias: 'FT Deployment Manual'})
                   } else if(job.name === 'EDG-SIT-Deploy' && !jobs.get('EDG-SIT-Deploy')){
                    jobs.set(job.name,{name: job.name,jobId: job.id,status: job.status,webUrl: job.web_url,packages: [],alias: 'EDG SIT Deployment Pipeline'})
                   } else if(job.name === 'EWI-SIT-Deploy' && !jobs.get('EWI-SIT-Deploy')){
                    jobs.set(job.name,{name: job.name,jobId: job.id,status: job.status,webUrl: job.web_url,packages: [],alias: 'EWI SIT Deployment Pipeline'})
                   } else if(job.name === 'build new packages' && !jobs.get('build new packages')){
                    jobs.set(job.name,{name: job.name,jobId: job.id,status: job.status,webUrl: job.web_url,packages: [],alias: 'Package Build Job'})
                   }
                }      
            }
        }
            ...
            if (gitLabResponse.status === 200 && gitLabResponse.data) {
                ...
                if(Array.isArray(formatedResp)){
                 ...
                 for (const line of formatedResp){
                    ...
                    if(parsedLine?.metric === 'sfpowerscripts.build.failed.packages' || parsedLine?.metric === 'sfpowerscripts.package.installation.failure'){
